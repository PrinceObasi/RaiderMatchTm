 ...
 *   USE_PLAYWRIGHT=true           # optional, enables headless fallback thats what
 */
...
function htmlToTextPreserveLists($root) {
  let text = $root.clone().find("script,style,noscript,iframe,svg").remove().end().text();
...
function pickDescriptionNode($, url) {
  ...
  if (/workday|myworkdayjobs\.com/.test(host)) {
    return $(
      '[data-automation-id="jobPostingDescription"], [data-automation-id="sections"], main, [role="main"]',
    ).first();
  }
...
async function enrichOneRow(row) {
  ...
  const { error } = await supabase.from("internships").update({ summary_text: trimmed, tech_stack }).eq("id", row.id);
